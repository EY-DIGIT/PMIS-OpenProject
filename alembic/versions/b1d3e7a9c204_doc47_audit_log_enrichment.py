...
The ``project_audit_logs`` table previously carried only the foreign-key
references (``project_id``, ``actor_id``) and the action / before /
after payload. Doc 47 enriches it with snapshotted, NOT-NULL columns so
