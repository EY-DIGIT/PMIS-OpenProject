...
def _add_assigned_to(inspector, table: str) -> None:
    ...
    if "assigned_to" not in cols:
        # Add as a plain String(36) first so the column exists; the FK is
        # added separately below to keep it optional / explicit.
        op.add_column(
            table,
    ...
    if fk_name not in fks:
        # SQLite doesn't support adding FKs after table creation in
        # alembic batch mode by default; guard so dev / test SQLite
        # bootstrap doesn't crash. On Postgres this just runs.
        bind = op.get_bind()
...
def downgrade() -> None:
    """Intentional no-op — keeps the new column for safety, mirroring
    the priority migration's downgrade choice."""
    pass
