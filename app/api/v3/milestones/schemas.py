...
class MilestoneCreateRequest(BaseModel):
    ...
    # Doc 41 follow-up: priority code — REQUIRED on create. Wire string is
    # validated against the in-code fallback here (cheap); the service
    # layer re-validates against the live DB catalog so admin-added codes
    # (p4, p5, …) are also accepted. Independent per-level — milestone
    # priority is not derived from / constrained by activity priority.
    priority: str = Field(
