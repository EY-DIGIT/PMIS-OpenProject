...
def _gate_milestone_revert_against_children(
    ...
    Together with the forward gate, this enforces the invariant that
    a milestone's status can never disagree with the status of its
    descendants — completion flows bottom-up, un-completion flows
    bottom-up too (children must be reverted before the parent).

    Only fires on the not_completed transition; flipping to other
