...
def list_project_audit_logs(
    ...
    # Build the label index + dependency maps once per page — same
    # pattern the tree endpoint uses (4 + 4 queries, all by project_id).
    # Each audit row references at most one M/A/T/S entity; we look up
    # its current displayCode + dependsOnDisplay from these maps.
