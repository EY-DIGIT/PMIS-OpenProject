...
def _gate_subtask_revert_against_parent(
    ...
    """Tester-feedback fix (top-down uncomplete): block flipping a
    subtask to ``not_completed`` while its parent is still
    ``completed``. The parent is another subtask when
    ``parent_subtask_id`` is set (nested subtask), otherwise it's the
    task referenced by ``task_id`` (top-level subtask).

    Replaces the previous reverse-children gate, which was a no-op for
    leaf subtasks (no nested → nothing to check) and let leaf reverts
    produce the inconsistent state documented in the bug report.
...
def update_subtask(
    ...
    *,
    ...
    if status is not None:
        ...
        # this subtask while its parent (nested S → parent S, or
        # top-level S → parent T) is still completed.
