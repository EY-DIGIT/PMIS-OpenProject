...
class ProjectAuditLogModel(Base):
    ...
    ``project_name``, ``project_status``, ``owner`` are all NOT NULL —
    the audit row is rejected at insert if the writer can't supply
    them. ``before`` / ``after`` stay nullable because create- and
    delete-class actions don't have one of those halves by design.
    """
