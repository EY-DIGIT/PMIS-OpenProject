...
class ProjectAuditLogRepository:
    ...
    def add(
        ...
        Doc 47: ``actor_login`` / ``project_name`` / ``project_status`` /
        ``owner`` are denormalized snapshots — the caller (typically
        :func:`record_audit` in projects/services/audit.py) looks them
        up against the live ``users`` and ``projects`` tables before
        invoking this repo. We do a defensive resolve here too in case
        a call site forgot — the audit table's NOT NULL columns must
