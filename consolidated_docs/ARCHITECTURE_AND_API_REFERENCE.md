...
| 37 part 2 | **User-management microservice extraction** — PMIS-user-management (port 8001) brought to monolith parity (commits `f840fde` foundation + `19a30e5` parity). Monolith adds `UserServiceProxyMiddleware` mounted in `app/main.py`; when `USER_SERVICE_PROXY_ENABLED=true` + `USER_SERVICE_URL` set, intercepts `/api/v3/users/*` + `/api/v3/master/{roles,permissions,notification_templates}/*` and forwards to port 8001. Fail-closed 503 on unreachable. 18 proxy tests (`test_doc37_user_service_proxy.py`). Full monolith suite: 881 passed, 3 skipped. Cutover runbook in `planned_changes/37`. |
