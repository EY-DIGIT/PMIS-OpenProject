...
| 37 part 2 (planned) | User-service microservice extraction. PMIS-user-management (port 8001) brought to parity with monolith; monolith proxies `/api/v3/users/*` + `/api/v3/master/{roles,permissions,notification_templates}/*` to port 8001 via `USER_SERVICE_PROXY_ENABLED` flag. Same SECRET_KEY, shared Postgres. See [planned_changes/37](../planned_changes/37.%20Static%20data%20to%20master%20endpoints%20%2B%20user-service%20microservice%20extraction.md). |
