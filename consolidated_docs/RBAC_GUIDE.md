...
**Last refresh**: 2026-05-10 (post-doc-44 round 9 — tester-feedback BE fixes shipped: F1 read-side hierarchy gate on `GET /users/{id}` so admin can no longer read super_admin profile; `PATCH /vendors/{id}` body-shape gate widened to a tier-aware allowlist (OA may edit Contact / Email / Mobile / Project Mapping / User Assignments; PA limited to User Assignments; admin still has no field restrictions); `PATCH /users/{id}` accepts `projectIds` for round-trip parity with CREATE + GET (replace semantics — `None` no-op, `[]` clear, `[...]` diff). Round 9 spec doc: `planned_changes/45.*`. See round 8 doc for the prior round's narrative; the round-by-round cumulative narrative for rounds 2–9 is documented across `planned_changes/44.*` and `planned_changes/45.*`.)
...
There are **six seeded roles** as of doc 43 round 4: `admin`, `super_admin`, `org_admin`, `project_admin`, `project_member`, `division_member`. The pre-doc-41 legacy roles (`member`, `viewer`, `vendor`) were retired in round 4 — the boot-time seed no longer creates them, and a guarded cleanup loop deletes any drifted rows on every startup (skipped + warned if any user still holds the role). The `admin` and `super_admin` roles are **auto-synced** to hold every registered code on every boot, and `admin` is **protected** from being deleted, renamed, or having its permission set modified through the API. **Scope** is carried on assignment rows in `user_role_assignments` (`organization_id?` or `project_id?`, exclusive of each other; both NULL means global).
...
(Doc 43 round 4 retired `member`, `viewer`, and `vendor` — see § 11 for the migration map and §6 for the cleanup behaviour.)
...
| `admin` | global | Everything except granting `super_admin`. Auto-synced to hold every code in `ADMIN_FULL_ROLE_PERMISSIONS` (= every code minus `users:grant_superadmin`) on every boot. **Pre-doc-43 the bootstrap admin user was auto-created on every boot — that auto-create is GONE post-doc-43.** Existing admin rows are preserved; new deploys must promote operators via super_admin. **Doc 44 round 5 re-locked the admin tier**: admin can no longer grant peer admin (only super_admin can). **Doc 44 round 7**: admin → admin mutations (PATCH / password / DELETE) are blocked on ALL ops, not just destructive — only super_admin can manage another admin. | Cannot grant `super_admin` (the role is API-ungrantable per round 7 — `init_db` only). Cannot grant or modify another admin (round 5 + round 7). Cannot PATCH / password-change / DELETE a super_admin user (F1 hierarchy gate). Cannot self-deactivate (G1). Admin role row is locked from delete / rename / permission-set mutation through the API. |
| `org_admin` (doc 41) | scope=org (vendor) | Manage user / project memberships within their owning vendor. `RBAC_ASSIGN` is granted, with the caller-vs-target gate restricting grants to `project_admin` / `project_member` / `division_member` on projects whose owning vendor matches the org_admin's `organization_id`. **Round 7**: full task / subtask CRUD (`tasks:delete`, `subtasks:delete` added) on projects within their vendor. **Round 8**: `master_data:view` stripped from the seed (no Master Data access). Vendor write surface (`PATCH /vendors/{id}`) accepts `user_assignments`-only bodies via `rbac:assign` carve-out; any non-assignment field (name, status, …) still requires `vendors:manage`. The list-scope filter on `GET /users` caps them to their own-vendor users; `GET /vendors/{id}` is now scope-checked too — no cross-vendor reads. | Cannot create / edit / delete vendor identity fields (only super_admin / admin do). Cannot create / update / delete user records (round 5: only super_admin / admin can). Cannot publish / close / delete projects. Cannot grant org_admin / admin / super_admin. Cannot read `master_data:*` (round 8). |
| `project_admin` (doc 41) | scope=project | Manage tasks/subtasks + project-membership on **the specific project the assignment carries**. Round-7 spec: cannot change roles, cannot self-unassign from project. Caller-vs-target rules let them grant `project_member` (only) on that project. **Round 8**: gains `users:read_all` so the FE can render the Search User page with their vendor's roster (round-7 vendor-scope filter on `GET /users` still caps them to own-vendor users). | Cannot create projects, cannot grant project_admin (only project_member), cannot touch master data or RBAC outside their project, cannot self-unassign from `project_members` (round 7 guard in `ProjectMembersController.remove_member`). |
| `project_member` (doc 41) | scope=project | Read project + its M/A/T/S, contribute task/subtask updates, comment, upload/download attachments. **Round 7**: full task / subtask CRUD (`tasks:delete`, `subtasks:delete` added) — "only add/edit/delete tasks and sub-tasks on the projects assigned to them". | No `users:read` / `vendors:read` (User Mgmt + Vendor Mgmt menus invisible to them). Cannot publish / close projects, cannot grant any role, cannot create or delete milestones / activities. |
...
The seeded permission lists for the doc-41 scoped roles live in [`app/core/permissions.py`](../app/core/permissions.py) (`SUPER_ADMIN_ROLE_PERMISSIONS`, `ADMIN_FULL_ROLE_PERMISSIONS`, `ORG_ADMIN_ROLE_PERMISSIONS`, `PROJECT_ADMIN_ROLE_PERMISSIONS`, `PROJECT_MEMBER_ROLE_PERMISSIONS`, `DIVISION_MEMBER_ROLE_PERMISSIONS`). The seed loop in `RbacRepository.sync_builtin_permissions` upserts them on every boot — if you add a new role-bundle entry there, restart the app and the seeded role gains the code.

The role names live as constants: `ADMIN_ROLE_NAME`, `SUPER_ADMIN_ROLE_NAME`, `ORG_ADMIN_ROLE_NAME`, `PROJECT_ADMIN_ROLE_NAME`, `PROJECT_MEMBER_ROLE_NAME`, `DIVISION_MEMBER_ROLE_NAME`. The `admin` and `super_admin` names are what the lockout protections (§6) check against.
...
|---|---|
| `super_admin` | every role tier **except** `super_admin` itself. **Round 7 — top-tier closure**: the `super_admin` role is API-ungrantable on every path (`can_caller_grant` returns "init_db only" regardless of caller). Bootstrap path is `app/infrastructure/db/session.py` at first boot. |
| `admin` | any role **except** `super_admin` and `admin` (round 5 reverted round-2 peer-grant relaxation; round 7 extended the block to non-destructive PATCHes too). |
| `org_admin` of vendor X | `project_admin` / `project_member` / `division_member` on projects whose owning vendor is X. |
| `project_admin` of project P | `project_member` on P only. **Cannot grant `project_admin`** (round 7 spec: "Cannot change roles"). |
| anyone else | nothing |

The same matrix is exposed read-only to the FE via `GET /api/v3/role-grants/{role_name}` (added round 7) so role-picker dropdowns can be scoped to the caller's tier without hard-coding the matrix in the FE.
...
| GET | `/api/v3/projects/{id}/role-assignments` | Per-project drill-down view, grouped by role bucket. Powers the FE Project-Mapping mock. | **Yes** — round-8 added a monolith mirror at the same path. The FE can hit either service; both read the shared `user_role_assignments` table. |
...
| ~~G4 peer-admin password change~~ | ~~admin → another admin~~ | RESTORED + extended in doc 44 round 7. See "Admin-peer block (round 7)" below. | (history) |
| ~~G5 peer-admin DELETE~~ | ~~admin → another admin~~ | RESTORED + extended in doc 44 round 7. See "Admin-peer block (round 7)" below. | (history) |
| **Admin-peer block (round 7)** | admin → another admin via PATCH `/users/{id}` (any field), `PATCH /users/{id}/password`, or `DELETE /users/{id}` | 403 "Only super_admin can manage another admin." | doc 44 round 7 — extends round-5 destructive-only block to ALL mutations on a peer admin. Implementation: `can_caller_modify_user` in `role_assignments/services.py`. |
| **Top-tier closure (round 7)** | grant of `super_admin` via any role-assignment route, by any caller | 403 "The super_admin role cannot be granted through the API." | doc 44 round 7 — `init_db` is the only path that mints another super_admin. |
| **Vendor-scope GET (round 8)** | non-admin caller `GET /api/v3/vendors/{id}` for a vendor other than their own | 403 "You can only view your own organization's details." | doc 44 round 8 — `vendors/routes.py::get_vendor`. |
| **Pre-publish project hide (round 8)** | non-admin caller `GET /api/v3/projects` lists / `GET /projects/{id}` fetches a project with `status in {draft, new}` | filtered out of list; 404 on single fetch | doc 44 round 8 — `projects/services/list.py` + `projects/controller.py::get`. |
| **Vendor PATCH tier-aware allowlist (round 8 + round 9)** | non-admin caller `PATCH /api/v3/vendors/{id}` with a field outside their tier's allowlist | 403 "Insufficient permissions to edit \<fields\>. Required: vendors:manage" | doc 44 round 8 introduced the body-shape carve-out; round 9 widened it. **Tier matrix**: admin / super_admin (`vendors:manage`) → no restrictions. org_admin (`rbac:assign` + holds `org_admin` role + same vendor) → `email`, `contact_person`, `phone_number`, `project_ids`, `user_assignments`. project_admin (`rbac:assign` + same vendor) → `user_assignments` only. Path: `vendors/routes.py::update_vendor`. |
| **F1 read-side hierarchy gate (round 9)** | non-super_admin caller `GET /api/v3/users/{sa_id}` where target holds super_admin and caller != target | 403 "Only super_admin can view a super_admin user." | doc 44 round 9 — `users/controller.py::UserController.get`. Round 7 covered PATCH/password/DELETE; round 9 closes the read-side gap. Self-fetch + super_admin → super_admin reads still allowed (parity with G2/G3, which only block destructive peer ops). |
| **Project_admin self-unassign block (round 7)** | project_admin caller `DELETE /api/v3/memberships/{id}` on their own membership row | 403 "...cannot unassign themselves..." | doc 44 round 7 — `ProjectMembersController.remove_member`. |
...
- **Doc 44 (SHIPPED through round 8, 2026-05-09)**: FE-aligned RBAC overhaul + alignment audit. Rounds 2–6 reshaped the doc-41 RBAC to match what the FE actually consumes (admin tier opened then re-locked; project mapping optional; caller-scoped lists; per-role perm overhaul; vendor `user_assignments` matrix). Round 7 audit closed the remaining gaps (top-tier closure for super_admin, admin-peer block extended, vendor-scope on `/users` + `PATCH /users/{id}`, task/subtask delete for org_admin + project_member, project_admin self-unassign block, `orgRole` on `/users` list, new `/role-grants/{role}` matrix lookup). Round 8 implemented the BE half of an FE bug-list audit: `GET /vendors/{id}` vendor-scope check, `GET /projects` filters out `draft`/`new` for non-admin, `master_data:view` stripped from org_admin, `PATCH /vendors/{id}` body-shape carve-out (org_admin can manage `user_assignments` without `vendors:manage`), monolith mirror of `GET /projects/{id}/role-assignments`, `users:read_all` added to project_admin. See `planned_changes/44.*` for the full round-by-round narrative + outcome.
- **Doc 45 (SHIPPED 2026-05-10, round 9)**: tester-feedback round on top of doc 44. F1 read-side hierarchy gate on `GET /users/{id}`. Vendor PATCH body-shape gate widened to a tier-aware allowlist (OA → Contact/Email/Mobile/ProjectMapping/UserAssignments; PA → UserAssignments only; admin → no restriction). `PATCH /users/{id}` accepts `projectIds` for round-trip parity with CREATE + GET (replace semantics). Plus an audit of GET/PATCH alignment across resources — see `planned_changes/45.*` appendix for deferred items (post-doc-38 stale fields on `projects` / `activities` / `tasks` / `subtasks`, two `User` shapes between monolith and user-mgmt, `project_members` schema using untyped dicts).
...
| 403 + "Only super_admin can manage another admin." | Round-7 admin-peer block. Use a super_admin account to make the change. |
| 403 + "The super_admin role cannot be granted through the API." | Round-7 top-tier closure. Bootstrap super_admin via `init_db` only; there is no API path. |
| 403 + "You can only view your own organization's details." | Round-8 vendor-scope check on `GET /vendors/{id}`. Non-admin callers can only fetch their own vendor. |
| 404 (single project) when caller is non-admin | Round-8 pre-publish hide. Project is in `draft`/`new` status; non-admin callers can't see it until it transitions to `published`. |
| 403 + "Insufficient permissions. Required: vendors:manage" on `PATCH /vendors/{id}` from org_admin | Round-8 body-shape gate. **Round 9 widened**: OA may now edit `email` / `contact_person` / `phone_number` / `project_ids` / `user_assignments` on their own vendor; only `name` / `description` / `active` still require `vendors:manage`. PA still limited to `user_assignments` only. |
| 403 + "Insufficient permissions to edit \<fields\>. Required: vendors:manage" on `PATCH /vendors/{id}` from OA | Round-9 tier-aware allowlist. Body included a field outside OA's allowed set. Drop the offending field(s) and retry. |
| 403 + "Only super_admin can view a super_admin user." | Round-9 F1 read-side gate. Use a super_admin account to read super_admin profiles. |
