...
**Last refresh**: 2026-05-09 (post-doc-44 round 8 — alignment-audit BE fixes shipped: `GET /vendors/{id}` vendor-scope check, non-admin sees only post-publish projects, `master_data:view` stripped from org_admin, body-shape carve-out on `PATCH /vendors/{id}` so org_admin can manage `user_assignments` without `vendors:manage`, monolith-side mirror of `GET /projects/{id}/role-assignments`, `users:read_all` added to project_admin. Also retro-documents rounds 3–7: project mapping optional on user create, caller-scoped list filters, per-role permission overhaul + `users:deactivate`, vendor `user_assignments` matrix on PATCH, top-tier closure (super_admin ungrant-able), admin-peer block extended, vendor-scope on `GET /users` + `PATCH /users/{id}`, `tasks:delete`/`subtasks:delete` for org_admin + project_member, project_admin self-unassign block, `orgRole` on list response, new `GET /role-grants/{role_name}` matrix lookup.)
...
| **Vendor PATCH body-shape gate (round 8)** | caller `PATCH /api/v3/vendors/{id}` with any field other than `user_assignments` while lacking `vendors:manage` | 403 "Insufficient permissions. Required: vendors:manage" | doc 44 round 8 — `vendors/routes.py::update_vendor`. The `user_assignments`-only carve-out passes with `rbac:assign` + same-vendor caller. |
...
| 403 + "Insufficient permissions. Required: vendors:manage" on `PATCH /vendors/{id}` from org_admin | Round-8 body-shape gate. Limit the body to `user_assignments` only — non-assignment fields require `vendors:manage`. |
