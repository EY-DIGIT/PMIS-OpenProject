...
Last run: **847 passed, 3 skipped** (full suite, post-doc 36 — notification templates + division contact required shipped).
...
| `test_projects.py` | Project CRUD, single-tier lifecycle (post-doc-33), **past-startDate test** (doc 24 part 1), inclusive-date validation |
| `test_baseline_version_propagation.py` | **Mostly obsolete post-doc-33 change 1** — versioning was removed. The remaining assertions exercise live-project lock guards. |
...
| `test_doc36_notification_templates.py` | **Doc 36** — DB-backed notification templates: 17 tests covering seeded built-in rows, full CRUD over `/api/v3/master/notification_templates/*`, active-uniqueness 409 guard, placeholder validation for the well-known kinds (`otp_login` / `password_reset_link` / `password_reset_otp`), built-in protect-from-delete, member-cannot-create permission gate, renderer DB-lookup substitution, and the generic-body fallback when no active row matches. The `seed_builtin_divisions` test fixture in `test_master_data.py` was updated to populate `email` + `phone_number` (NOT NULL post-doc-36); divisions test class additions cover required-on-create, validation, and empty-string-rejected-on-patch. |
...
- **Project lifecycle**: create / update / publish / save / close / soft-delete with cascade. Single-tier post-doc-33 — `suspend` and `versions/create` are gone; `published → draft` revert is now legal.
...
- **Audit trail** for project / M / A / T / S writes + dep-edge changes via `project_audit_logs`. Doc 33 added `actor_role` so each audit row records the role bucket the actor occupied (admin / member / vendor / viewer).
