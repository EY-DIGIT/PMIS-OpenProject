...
class TestRoundTrip:
    def test_task_assigned_round_trips(
        self, client, admin_user, admin_headers, alice_user,
    ):
        _pid, _vid, _m, a = _setup_published_activity(client, admin_headers)
...
class TestInvalidAssigneesRejected:
    ...
    def test_task_inactive_user_rejected_on_create(
        self, client, admin_user, admin_headers, inactive_user,
    ):
        _pid, _vid, _m, a = _setup_published_activity(client, admin_headers)
    ...
    def test_task_deleted_user_rejected_on_create(
        self, client, admin_user, admin_headers, deleted_user,
    ):
        _pid, _vid, _m, a = _setup_published_activity(client, admin_headers)
...
class TestIndependence:
    def test_each_level_holds_its_own_assignee(
        ...
        _pid, _vid, _m, a = _setup_published_activity(client, admin_headers)
        t = _make_task(
            client, admin_headers, a["id"], assigned_to=admin_user.id,
        ).json()["data"]
        s = _make_subtask(
            client, admin_headers, t["id"], assigned_to=alice_user.id,
        ).json()["data"]
        n = _make_nested(client, admin_headers, s["id"]).json()["data"]
        assert t["assignedTo"] == admin_user.id
...
class TestTreeSurfaces:
    def test_tree_emits_assignedTo_on_t_s_nested(
        ...
        assert t_node["assignedTo"] == admin_user.id
        assert t_node["assignedToName"] == "Admin User"
        ...
        # M / A intentionally do NOT carry the field — we don't want
        # the FE rendering an "assigned to" pill at those levels.
