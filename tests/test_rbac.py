...
class TestMePermissions:
    ...
    def test_member_sees_baseline_set_only(self, client, member_user, member_headers):
        """Post-doc-43-round-4 the 'member' role bundle is gone; the
        ``member_user`` fixture grants a baseline as direct
        user_permissions. The /me/permissions endpoint surfaces them
        the same as role-derived perms."""
        resp = client.get(
...
class TestRoleProtection:
    ...
    def test_custom_role_can_be_deleted(
        ...
        """Custom (non-builtin) roles can be deleted. Contrast with
        admin / super_admin which are locked."""
        create = client.post(
            "/api/v3/master/roles/create",
...
class TestUserRoleAssignment:
    def test_assign_role_to_user(
        ...
        """Assign a custom role via the legacy per-user-role endpoint.
        (Pre-doc-43-round-4 this used the seeded 'viewer' role.)"""
        create = client.post(
            "/api/v3/master/roles/create",
...
class TestDirectUserPermissions:
    def test_direct_permission_grants_access(
        ...
        # Create a no-role user; verify they can't create a project.
        bare, headers = _bare_user(db_session, "noperms")
        resp = client.post(
            "/api/v3/projects/create",
            json={"name": "X", "owner": "tmd1"},
            headers=headers,
        )
        assert resp.status_code in (401, 403)

        # Grant projects:create directly. Then they CAN see the perm reflected.
        # (The actual project create endpoint has further validation; we just
        # assert that the permission gate stops blocking.)
        resp = client.post(
            f"/api/v3/users/{bare.id}/permissions/projects:create",
            headers=admin_headers,
        )
        assert resp.status_code == 200, resp.text
        # /me/permissions reflects it.
