...
class TestMilestoneCreateUnifiedShape:
    def test_create_with_status_and_dependsOn(self, client, admin_user, admin_headers):
    ...
    def test_omitted_fields_use_defaults(self, client, admin_user, admin_headers):
        pid, _vid = _setup_project_and_vendor(client, admin_headers)
        r = client.post(
            f"/api/v3/projects/{pid}/milestones/create",
            headers=admin_headers,
...
class TestActivityCreateUnifiedShape:
    def test_create_with_full_payload(self, client, admin_user, admin_headers):
